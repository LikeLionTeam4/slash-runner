...
사용자 PC에 설치돼 **클라우드가 보낸 작업을 로컬에서 실행하고 결과만 돌려주는** 프로그램입니다.
메뉴바(macOS)·시스템 트레이(Windows) 앱으로 동작합니다.

**처리하는 작업 5종**

| 작업 | 명령 | 하는 일 |
|---|---|---|
| `FILE_SEARCH` | `/file` | 등록한 검색 폴더에서 파일 찾기 (SQLite FTS5) |
| `FILE_OPEN` | `/open` | 검색 결과 파일의 **위치를** 탐색기로 표시 |
| `SYSTEM_STATUS` | `/status` | CPU·메모리·디스크 사용량 (psutil) |
| `CODE_ANALYSIS` | `/code` | 등록한 프로젝트 폴더를 로컬 CLI로 **읽기 전용** 분석 |
| `AI_AGENT_USAGE` | `/usage` | 로컬 Claude Code·Codex의 토큰 사용량 조회 |

`TEXT_SUMMARY`(`/summary`)도 구현돼 있으나 **현재 서버에서 이 경로로 보내지 않습니다** —
`slash-api`의 `slash.text-summary.runner-enabled`가 기본값 `false`입니다. 인젝션 방어가
완성되기 전까지 닫아 두기로 한 것입니다(자세한 것은 아래 "메시지 프로토콜" 절).

**설계 원칙 네 가지**

- **인바운드 포트를 열지 않습니다.** PC에서 클라우드로 나가는 WSS 연결만 사용합니다.
  방화벽·포트포워딩 설정이 필요 없고, 외부에서 이 PC로 들어올 경로가 없습니다
- **등록한 폴더만 접근합니다.** 사용자가 관리 창에서 등록한 검색 폴더·프로젝트 폴더
  바깥은 매 실행 시 경계를 재검증해 차단합니다. 등록을 해제하면 재시작 없이 즉시 반영됩니다
- **결과만 올려보냅니다.** 파일 절대 경로는 클라우드로 가지 않습니다. 검색 결과는 상대
  경로와 불투명 식별자(`fileRef`)만 담고, 그 값으로 실제 경로를 되찾는 일은 이 PC가 합니다
- **로컬 CLI는 읽기 전용으로 실행합니다.** 쓰기·셸 실행 도구를 CLI 플래그로 차단합니다.
  다만 그 보장이 플래그 자체에 의존한다는 것이 알려진 한계입니다(아래 "알려진 한계" 절)

**기기 신원** — Ed25519 키 쌍으로 등록하며 **개인키는 이 PC를 벗어나지 않습니다**.
OS 보안 저장소(macOS Keychain, Windows Credential Manager)에 보관하고 서버에는 공개키만
올라갑니다.
...
> 단독으로 변경할 수 없습니다. 실제로 서버에 전송하는 **값**은 `slash-pc-runner-py/0.5.8`처럼
> 패키지 버전을 그대로 담습니다.
