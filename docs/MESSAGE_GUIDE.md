...
8종(`DEVICE_BUSY, TASK_TYPE_NOT_SUPPORTED, INVALID_PARAMETERS, SEARCH_FOLDER_NOT_FOUND,
WORKSPACE_NOT_FOUND, CODE_AGENT_NOT_CONFIGURED, TASK_EXPIRED, POLICY_DENIED`)이며, 이 PC 작업
실행기는 `SYSTEM_STATUS`/`FILE_SEARCH` 두 가지만 지원하므로 실제로는 이 중 일부만 발생한다.
