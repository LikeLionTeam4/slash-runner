...
class FileIndexStore:
    ...
    def _upsert_file(self, folder_id: str, root: Path, full_path: Path) -> None:
        ...
        with self._lock:
            self._conn.execute(
                """INSERT INTO files (folder_id, relative_path, name, size_bytes, modified_at)
                   VALUES (?, ?, ?, ?, ?)
                ...
                (folder_id, relative_path, name, stats.st_size, modified_at),
            )
            self._conn.commit()
    ...
    def search(self, folder_id: str, query: str, limit: int = 20) -> dict:
        ...
        with self._lock:
            if len(normalized) >= 3:
                ...
                rows = self._conn.execute(
                    """SELECT f.name, f.relative_path, f.size_bytes, f.modified_at
                ...
                rows = self._conn.execute(
                    """SELECT name, relative_path, size_bytes, modified_at
        ...
        items = [
            {"name": row[0], "relativePath": row[1], "sizeBytes": row[2], "modifiedAt": row[3]}
            for row in rows[:limit]
        ]
        return {"items": items, "returnedCount": len(items), "truncated": truncated}
